...
@Component
public class AssessmentAreaUploadProcessor implements IUploadProcessor
{

	private static Logger LOGGER  = Logger.getLogger(AssessmentAreaUploadProcessor.class);
...
//			Customer	 AssessmentAreaName	 AssessmentAreaDescription	 ParentAssessmentAreaName
			...
			String assessmentAreaDescription = row.getColValue(AssessmentAreaUploadCsvhandler.AA_DESCRIPTION_KEY);
			String parentAssessmentAreaName = row.getColValue(AssessmentAreaUploadCsvhandler.AA_PARENT_KEY);
			if(StringUtils.isEmpty(parentAssessmentAreaName))
			{
				parentAssessmentAreaName = null;
			}

			assessmentAreas.add(new AssessmentArea(assessmentAreaName, assessmentAreaDescription, parentAssessmentAreaName, customer));
		}
		m_assessmnetAreaDao.addAssessmentAreas(assessmentAreas);
